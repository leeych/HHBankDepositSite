    ...
    public partial class BankRateManage : System.Web.UI.Page
    {
        ...
        private void GenAllBankRateRecord()
        {
            ...
            for (int i = 0; i < rateList.Count; i++)
            {
                ...
                bankRateTable.Rows[i + 1].Cells[1].Text = (rateList[i].Rate.CurrRate*100).ToString("f5");
                bankRateTable.Rows[i + 1].Cells[2].Text = (rateList[i].Rate.M03*100).ToString("f5");
                bankRateTable.Rows[i + 1].Cells[3].Text = (rateList[i].Rate.M06*100).ToString("f5");
                bankRateTable.Rows[i + 1].Cells[4].Text = (rateList[i].Rate.Y01*100).ToString("f5");
                bankRateTable.Rows[i + 1].Cells[5].Text = (rateList[i].Rate.Y02*100).ToString("f5");
                bankRateTable.Rows[i + 1].Cells[6].Text = (rateList[i].Rate.Y03*100).ToString("f5");
                bankRateTable.Rows[i + 1].Cells[7].Text = (rateList[i].Rate.Y05*100).ToString("f5");
            }
        }
        ...
        private void InsertTableRow(BankRateInfo rateInfo, int row)
        {
            if (bankRateTable.Rows.Count > 10)
            {
                ...
                m03Cell.Text = (rateInfo.Rate.M03 * 100).ToString("f5");
                TableCell m06Cell = new TableCell();
                m06Cell.Text = (rateInfo.Rate.M06 * 100).ToString("f5");
                TableCell y01Cell = new TableCell();
                y01Cell.Text = (rateInfo.Rate.Y01 * 100).ToString("f5");
                TableCell y02Cell = new TableCell();
                y02Cell.Text = (rateInfo.Rate.Y02 * 100).ToString("f5");
                TableCell y03Cell = new TableCell();
                y03Cell.Text = (rateInfo.Rate.Y03 * 100).ToString("f5");
                TableCell y05Cell = new TableCell();
                y05Cell.Text = (rateInfo.Rate.Y05 * 100).ToString("f5");
            }
        }
